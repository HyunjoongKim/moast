			...
			<form:form commandName="searchVO" method="get" name="submitForm" id="submitForm" action="${path }/mo/history/preset/list.do">
				...
				<c:if test="${searchVO.popYn ne 'Y'}">
					<div class="card-header ui-sortable-handle pl-1 pr-1 pt-0">
						<div class="row">
							<div class="col-lg-6">
								<h3 class="card-title h3icn">공유받은 Preset</h3>
							</div>
							<div class="col-lg-6 text-right mt-2 location"><i class="fa fa-home"></i><i class="fa fa-chevron-right ml-2 mr-2"></i>분석히스토리<i class="fa fa-chevron-right ml-2 mr-2"></i>공유받은 Preset </div>
						</div>
					</div>
				</c:if>
				<!-- //title -->
			                                ...
			                                <tbody>
												...
												<c:forEach var="result" items="${resultList}" varStatus="status">
													<tr class="sprt_t">
														...
														<c:if test="${searchVO.popYn ne 'Y'}">
															<td> 
																<a href="${path}/mo/clinic/list.do?ps_idx=${result.ps_idx}" target="_self" class="btn btn-success btn-sm">New Preset</a>
															</td>
														</c:if>
														<td><a href="${path}/mo/history/presetshared/list_study.do?ps_idx=${result.ps_idx}&popYn=${searchVO.popYn}" target="_self" class="btn btn-primary btn-sm">Preset 선택</a></td>
													</tr>
													<tr>
														<td style="text-align:left"><c:out value="${result.ps_note}"/></td>

...
<script type="text/javascript">
	...
	$(function() {
		...
		$(document).on("click", ".comment-edit", function(){
			...
			if (newComment != null && newComment != undefined) {
				$.ajax({
					url: "${path}/mo/analysisdata/methylation/update.do",
					method: "post",
	...
	function viewMarker(meth_idx) {
		var url = "${path}/mo/analysisdata/methylation/read_marker.do?meth_idx=" + meth_idx
				
		window.open(url, 'Methylation_Read', 'height=820,width=850,resize=no')
	}
				...
				<div class="card-header ui-sortable-handle pl-1 pr-1 pt-0">
					<div class="row">
						...
						<div class="col-lg-6 text-right mt-2 location"><i class="fa fa-home"></i>
							...
							<i class="fa fa-chevron-right ml-2 mr-2"></i>History (Methylation)
						</div>
					</div>
				</div>
				<!-- //title -->
			                                ...
			                                <tbody>
												...
												<c:forEach var="result" items="${resultList}" varStatus="status">
													<tr>
														...
														<td style="text-align:left"><c:out value="${result.ilmnID}"/></td>
														<td style="text-align:left">
															<button type="button" class="btn btn-sm btn-success" onclick="viewMarker(${result.recordIdx})" target="Methylation_View">View Marker</button>
															<a class="btn btn-sm btn-success" href="${path}/mo/analysisdata/methylation/read_detail.do?meth_idx=${result.recordIdx}">View Detail</a>
														</td>
														<%-- <td style="text-align:left"><a class="btn btn-sm btn-success" href="#" onclick="openPopup(${result.recordIdx})" target="Methylation_View">View Marker</a></td> --%>

...
<script type="text/javascript">
	...
	function fn_delete(meIdx){
		...
		$.ajax({ 
	      	  url: path+"/adms/userdata/manage/delete_action.do", 
	      	  type: "POST", 
...
<form:form commandName="searchVO" method="get" name="listForm" id="listForm" action="${path}/adms/userdata/manage/list.do">
	<div class="panel">
		<div class="panel-body">
			<div class="form-inline" >
				<div class="form-group">
					<label class="control-label" for="">Title</label>
					<input type="text" id="searchUdTitle" name="searchUdTitle" value="${searchVO.searchUdTitle }" placeholder="Title" style="width:300px" class="form-control"  />
				</div>
				<div class="form-group m-l-10 m-r-10">
					<label class="control-label" for="">Approval status</label>
					<select name="searchUdStatus" id="searchUdStatus"  class="form-control" />
						<option value="">All</option>
						<c:forEach var="result" items="${statusList}" varStatus="status">
				       		<option value="${result.main_code }" ${result.main_code eq searchVO.searchUdStatus ? 'selected' : ''}>${result.code_name }</option>
						</c:forEach>
					</select>
				...
				<table id="datatable-scroller" class="table table-striped table-bordered text-center">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<span class=""><c:out value="${result.ud_statusName}"/></span>
							</td>
							<td>
								<a href="${path}/adms/userdata/manage/update.do?ud_idx=${result.ud_idx}&${qustr}" class="btn btn-success btn-sm">Select </a>
							</td>
						</tr>
	...
	<div class="pull-right">
		<div class="btn-group dropup">
			<a href="${path}/adms/userdata/manage/create.do" class="btn btn btn-primary btn-lg"><spring:message code="btn.create" text="등록" /></a>
		</div>
	</div>
	 --%>
	<div class="clearfix"></div>

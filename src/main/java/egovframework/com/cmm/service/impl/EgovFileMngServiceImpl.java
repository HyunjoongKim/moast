...
public class EgovFileMngServiceImpl extends AbstractServiceImpl implements EgovFileMngService {
    ...
    public List<FileVO> selectFileInfsByFileName(FileVO fvo) throws Exception {
	return fileMngDAO.selectFileInfsByFileName(fvo);
    }
    ...
    @Override
	public String getAtchNextFileId() throws Exception {
		return fileMngDAO.getAtchNextFileId();
	}

	@Override
	public int getSubFileTableRow(String atchFileId) throws Exception {
		return fileMngDAO.getSubFileTableRow(atchFileId);
	}

	@Override
	public void deleteFileTableRow(int remainCnt, FileVO fvo) throws Exception {
		try{
			if(remainCnt <=1 ){
				//둘다지우고
				fileMngDAO.deleteSubTable(fvo);
				fileMngDAO.deleteMainTable(fvo);
			}else{
				//서브만지운다.
				fileMngDAO.deleteSubTable(fvo);
			}
		}catch(Exception e){
			System.out.println("fileDB삭제 예외 : "+ e.toString());
		}
	}
	...
	public int isMainFileTableRow(String ATCH_FILE_ID, String SITE_CODE)
			throws Exception {
		return fileMngDAO.isMainFileTableRow(ATCH_FILE_ID,SITE_CODE);
	}

	@Override
	public void insertAtchFileId(FileVO vo) throws Exception {
		fileMngDAO.insertAtchFileId(vo);
	}

	@Override
	public void insertFileInfsDetails(List<FileVO> result, String menuType, String menuIdx) throws Exception {
		fileMngDAO.insertFileInfsDetails(result, menuType, menuIdx);
	}

	@Override
	public List<FileVO> selectFileInfsBySn(FileVO searchVO) throws Exception {
		return fileMngDAO.selectFileInfsBySn(searchVO);
	}
